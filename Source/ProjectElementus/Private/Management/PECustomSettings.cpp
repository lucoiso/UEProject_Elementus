...
void UPECustomSettings::ApplyNonResolutionSettings()
{
	...
	if (IConsoleVariable* AntiAliasingCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.AntiAliasingMethod")))
	{
		AntiAliasingCVar->Set(AntiAliasingMode, ECVF_SetByConsole);
	}

	if (IConsoleVariable* FSREnabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.FidelityFX.FSR.Enabled")))
	{
	...
	if (IConsoleVariable* TemporalUpsamplingCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.TemporalAA.Upsampling"));
		TemporalUpsamplingCVar && AntiAliasingMode == 2)
	{
		TemporalUpsamplingCVar->Set(bEnableTemporalUpscaling, ECVF_SetByConsole);
	}

	if (IConsoleVariable* DynamicGICvar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.DynamicGlobalIlluminationMethod")))
	{
		// 0 = None | 1 = Lumen
		DynamicGICvar->Set(bEnableLumen, ECVF_SetByConsole);
	}

	if (IConsoleVariable* DynamicGRCvar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.ReflectionMethod")))
	{
		// 1 = Lumen | 2 = Screen Space
		DynamicGRCvar->Set(bEnableLumen ? 1 : 2, ECVF_SetByConsole);
	}

	if (IConsoleVariable* VirtualShadowCvar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.Shadow.Virtual.Enable")))
	{
